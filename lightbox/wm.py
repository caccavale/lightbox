...
class WM(Thread):
    def __init__(self, Display): # Yeah its bad but Display != display
    ...
    def redirect_screen_events(self, screen_id):
        ...
        for window in root_window.query_tree().children:
            log.info('Grabbing mouse motion events for window {0}'.format(window))
            self.grab_window_events(window)

        return True

    def x_error_handler(self, err, request):
        log.error('X protocol error: {0}'.format(err))
    ...
    def handle_event(self):
        ...
        try:
            ...
            log.error('Display connection closed by server')
        ...
        if event.type in self.event_dispatch_table:
            ...
            log.warning('unhandled event: {event}'.format(event=event))
